...
- A setting to group the sidebar by project (Settings, Appearance, "Group threads by project"). Each project folds and shows how many of its threads are working, and a folded project still shows its orchestrator; settled threads stay in one list. Groups keep a fixed order that activity never changes; right-click a group header to move it.
