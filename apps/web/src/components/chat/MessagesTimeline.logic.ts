...
function singleToolCallLabel(entry: WorkLogEntry): string {
  ...
  return workEntryHeadingLabel(entry);
}
 ...
 * fleet cannot end up behind "+N tool calls"; a submitted answer, the record of
...
export function deriveMessagesTimelineRows(input: {
  ...
  for (let index = 0; index < input.timelineEntries.length; index += 1) {
    ...
    if (timelineEntry.kind === "work") {
      if (workEntryPinnedInGroup(timelineEntry.entry) || timelineEntry.entry.tone === "error") {
        nextRows.push({
          kind: "work",
          id: timelineEntry.id,
          createdAt: timelineEntry.createdAt,
      ...
      while (cursor < input.timelineEntries.length) {
        ...
        if (
          ...
          workEntryPinnedInGroup(nextEntry.entry) ||
