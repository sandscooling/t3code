...
describe("deriveMessagesTimelineRows", () => {
  ...
  it("keeps a submitted answer visible when later work folds", () => {
    // Upstream folds a whole work group behind one toggle, and an answer is
    // written mid-turn, so the fold would swallow it. Pinned like an
    // agent-spawn row: the decision keeps its own row and the work folds
    // around it.
    const rows = deriveMessagesTimelineRows({
      timelineEntries: [
        {
    ...
    expect(rows.map((row) => row.id)).toEqual(["answer-entry", "work-toggle:work-entry-1"]);
    expect(rows.find((row) => row.kind === "work-toggle")).toMatchObject({
      hiddenCount: 2,
    });
  });
