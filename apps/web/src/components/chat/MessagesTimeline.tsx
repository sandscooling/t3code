...
const PlainWorkEntryRow = memo(function PlainWorkEntryRow(props: {
  ...
  const submittedAnswers =
    workEntry.sourceActivityKind === "user-input.resolved"
      ...
      : null;
  // Upstream's `questionAnswer` is a different activity kind and carries the
  // files attached to an answer, so the two labels never contend for one row.
  const previewText = submittedAnswers
    ? workEntryHeadingLabel(workEntry)
  ...
  const viewedImage =
    viewedImagePath && threadRef
      ...
      : null;
  const canExpand =
    // A submitted answer renders below the row rather than inside its fold,
    // so an answered question never counts as expandable content.
