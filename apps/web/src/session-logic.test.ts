...
describe("deriveTurnInterruptionNotice", () => {
  ...
  it("returns no row while a turn is still running", () => {
    expect(
      deriveTurnInterruptionNotice({ ...interruptedTurn, state: "running", completedAt: null }),
    ).toBeNull();
  });

  it("returns no row for a turn that completed on its own", () => {
    expect(deriveTurnInterruptionNotice({ ...interruptedTurn, state: "completed" })).toBeNull();
  });
  ...
  it("falls back to earlier timestamps when the turn never recorded an end", () => {
    expect(deriveTurnInterruptionNotice({ ...interruptedTurn, completedAt: null })?.createdAt).toBe(
      "2026-02-27T21:10:01.000Z",
    );
    expect(
      deriveTurnInterruptionNotice({ ...interruptedTurn, completedAt: null, startedAt: null })
        ?.createdAt,
