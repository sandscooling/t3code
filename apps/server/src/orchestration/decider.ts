    ...
    case "thread.auto-settle": {
      ...
      if (command.type === "thread.settle") {
        for (const child of readModel.threads) {
          ...
          // Same eligibility test the settled thread itself gets above: an
          // explicit settle dismisses message-mode questions, while approvals
          // and native callbacks still count as blocked-on-you work.
          if (
            Array.from(openRequests(child).values()).some(
              (activity) =>
                activity.kind !== "user-input.requested" ||
                !Predicate.isObject(activity.payload) ||
                activity.payload.responseMode !== "message",
