...
describe("ProviderRuntimeIngestion", () => {
  ...
  it("projects rate limit updates into normalized thread activities", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
    ...
    const thread = await waitForThread(harness.readModel, (entry) =>
      entry.activities.some(
        (activity: ProviderRuntimeTestActivity) => activity.kind === "rate-limits.updated",
      ),
    );
  ...
  it("drops rate limit events that carry no usable window", async () => {
    const harness = await createHarness();

    harness.emit({
      type: "account.rate-limits.updated",
      eventId: asEventId("evt-account-rate-limits-empty"),
      provider: ProviderDriverKind.make("claudeAgent"),
      createdAt: "2026-01-01T00:00:00.000Z",
      threadId: asThreadId("thread-1"),
      payload: { windows: [] },
    });
    // An activity that would land afterwards proves the empty event was not
    // merely slow to arrive.
    harness.emit({
      type: "runtime.warning",
      eventId: asEventId("evt-after-empty-rate-limits"),
      provider: ProviderDriverKind.make("claudeAgent"),
      createdAt: "2026-01-01T00:00:01.000Z",
      threadId: asThreadId("thread-1"),
    ...
    const thread = await waitForThread(harness.readModel, (entry) =>
      entry.activities.some(
        (activity: ProviderRuntimeTestActivity) => activity.id === "evt-after-empty-rate-limits",
      ),
    );
