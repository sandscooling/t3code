...
const makeProviderService = Effect.fn("makeProviderService")(function* (
  ...
  // One credential carries every toolset the session is allowed, so the grant
  // is a set rather than a boolean. Browser access keeps its own per-project
  // answer above; orchestration is a plain server-wide switch, since an agent
  // that can spawn agents is a larger grant than one driving a browser tab.
  const grantedMcpCapabilities = (threadId: ThreadId): Effect.Effect<ReadonlySet<McpCapability>> =>
    Effect.gen(function* () {
      ...
      const orchestration = yield* serverSettings.getSettings.pipe(
        Effect.map((settings) => settings.enableAgentOrchestration),
        Effect.catch((cause) =>
          Effect.logWarning(
            "Could not read server settings; withholding agent orchestration for this session.",
            { cause },
          ).pipe(Effect.as(false)),
        ),
      );
      if (orchestration) capabilities.add("orchestration");
      return capabilities;
    });

  const prepareMcpSession = (threadId: ThreadId, providerInstanceId: ProviderInstanceId) =>
    Effect.gen(function* () {
      const capabilities = yield* grantedMcpCapabilities(threadId);
