...
const makeProviderService = Effect.fn("makeProviderService")(function* (
  ...
  // Orchestration is a plain server-wide switch rather than a per-project answer
  // like browser access, since an agent that can spawn agents is a larger grant
  // than one driving a browser tab.
  const agentOrchestrationEnabled = serverSettings.getSettings.pipe(
  ...
  const agentAccessCapabilities = Effect.fn("ProviderService.agentAccessCapabilities")(function* (
    ...
    if (yield* agentOrchestrationEnabled) capabilities.add("orchestration");
    return capabilities;
  });
