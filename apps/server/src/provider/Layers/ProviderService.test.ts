...
describe("agent browser access", () => {
      ...
      const providerLayer = makeProviderServiceLive({
        ...
        Layer.provide(
          ServerSettings.ServerSettingsService.layerTest({
            ...
            enableAgentOrchestration: options?.enableAgentOrchestration ?? false,
  ...
  // Orchestration is its own grant: it must not smuggle preview in with it.
  it.effect("adds orchestration to the credential only when that setting is on", () =>
    Effect.gen(function* () {
  ...
  it.effect("grants orchestration beside the other capabilities when every setting is on", () =>
    Effect.gen(function* () {
      const threadId = asThreadId("thread-every-grant-on");
