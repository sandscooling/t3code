        ...
        return built.handle("preview_snapshot", payload).pipe(
          ...
          Effect.flatMap(({ encodedResult }) =>
            Effect.gen(function* () {
              ...
              const metadata = { ...pickPreviewSnapshotSections(page, requested), ...extras };
              // Upstream's bounding reads every section, so it runs on an
              // unfiltered snapshot. A section filter is the agent sizing the
              // answer itself, and goes out as asked.
              const bounded =
                requested.size < selectPreviewSnapshotSections(undefined).size
                  ? { text: encodeJsonText(metadata), omitted: [] }
                  : boundSnapshotMetadata({ ...page, ...extras });
              return new McpSchema.CallToolResult({
                ...
                content: [
                  ...
                  // The section filter and includeImage compose: either one
                  // dropping the screenshot drops the image.
                  ...(requested.has("screenshot") && payload?.includeImage !== false
